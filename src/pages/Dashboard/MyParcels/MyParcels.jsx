...
const MyParcels = () => {
    ...
    queryFn: async () => {
      try {
        ...
        if (error.response?.status === 404) {
          // No parcels found — return empty list gracefully
          return [];
        }
        throw error;
      }
  ...
  const { mutateAsync } = useMutation({
    ...
    onSuccess: (data) => {
      ...
      MySwal.fire({
        title: '<div style="font-size: 1.5rem; font-weight: 700; color: #059669; margin-bottom: 8px;">Successfully Deleted!</div>',
        html: `
      <div style="text-align: center; padding: 8px 0;">
        ...
        <h3 style="font-size: 1.125rem; font-weight: 600; color: #374151; margin-bottom: 8px;">
          Parcel Deleted
        </h3>
        <p style="color: #6b7280; font-size: 0.95rem;">
          <strong>"${parcel.parcelName}"</strong> has been removed from the system.
        </p>
      </div>
    `,
        icon: 'success',
        ...
        background: '#ffffff',
        customClass: {
          popup: 'rounded-2xl p-6 shadow-2xl',
          confirmButton: 'px-6 py-3 font-semibold rounded-lg shadow-sm hover:shadow-md transition-all duration-200'
        },
        width: '420px',
        timer: 3000,
  ...
  const handleDelete = async (parcel) => {
    ...
    <div style="text-align: center; padding: 8px 0;">
      ...
      <h3 style="font-size: 1.125rem; font-weight: 600; color: #374151; margin-bottom: 8px;">
        Delete "${parcel.parcelName}"?
      </h3>
      <p style="color: #6b7280; margin-bottom: 4px; font-size: 0.95rem;">
        Tracking ID: <strong>${parcel.trackingId}</strong>
      </p>
      <p style="color: #ef4444; font-size: 0.875rem; font-weight: 500; margin-top: 12px;">
  ...
  `,
      ...
      customClass: {
        ...
        title: '!mb-0',
        confirmButton: 'px-6 py-3 font-semibold rounded-lg shadow-sm hover:shadow-md transition-all duration-200',
        cancelButton: 'px-6 py-3 font-semibold rounded-lg border border-gray-300 hover:bg-gray-50 transition-all duration-200'
      },
      width: '440px',
      reverseButtons: true,
      focusCancel: true,
